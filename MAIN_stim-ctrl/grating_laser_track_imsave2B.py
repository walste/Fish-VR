...
from pylab import *
from fishvr_utils_imsaving2B import *
...
def grab_frames(cam, save_queue, stop_requested, fit_queue):
        ...
        while not stop_requested.is_set():
            ...
            if save_queue is not None:
                save_queue.put( (buf_cropped.copy(),i,caption,vid_observer) )
                if i % 170000000:
                    print "save queue:", save_queue.qsize()
            if fit_queue is not None:
                fit_queue.put( (buf_cropped.copy(),i,caption) )
                if i % 170000000:
                    print "fit queue:", fit_queue.qsize()
...
def fit_thread_worker(p1, p2, p3, fit_queue, stop_requested,  results_fname, debugmode=True, memorize_frames=False):
    global i
    ...
    while True:
        ...
        try:
            ...
            if i%170000==0:
                file_results.close()
                file_results = open(results_fname, 'ab')
            #spkl.s_dump(result_tuple, file_results)
        except:
            print "I didn't dump this iteration's results"
            continue

        #if i == 60*100*3: # every 5 minutes
            #file_results.close()
            #file_results = open(results_fname, 'wb')    # This deletes the file and makes a new one
...
if __name__ == "__main__":
    ...
    #params.dur_CS = 2. *100
    params.vid_span = 15. * 100 #The observation should span 10-15s before and after the CS, which is being recorded.
    ...
    if starting_mode == 2:
        ...
        fit_thread = threading.Thread(target=fit_thread_worker, args=(p1, p2, p3, fit_queue, fit_thread_stop_requested, results_fname, False))
        fit_thread.start()
        grab_thread_stop_requested = threading.Event()
        grab_thread = threading.Thread(target=grab_frames, args=(cam, save_queue, grab_thread_stop_requested, fit_queue))
        grab_thread.start()
