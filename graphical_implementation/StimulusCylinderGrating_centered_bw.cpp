...
class StimulusCylinderGrating_centered_bw: public StimulusInterface
{
public:
    StimulusCylinderGrating_centered_bw();
    ...
    std::string name() const { return "StimulusCylinderGrating_centered_bw"; }
...
StimulusCylinderGrating_centered_bw::StimulusCylinderGrating_centered_bw() :
    _t0(-1)
{
    _virtual_world = create_virtual_world();
}

std::vector<std::string> StimulusCylinderGrating_centered_bw::get_topic_names() const
{
...
std::string StimulusCylinderGrating_centered_bw::get_message_type(const std::string& topic_name) const
{
    std::string result;
    if (topic_name=="grating_info") {
...
void StimulusCylinderGrating_centered_bw::update( const double& time, const osg::Vec3& observer_position, const osg::Quat& observer_orientation )
{
    double dt=0.0;
    if (_t0 > 0) {
...
osg::ref_ptr<osg::Group> StimulusCylinderGrating_centered_bw::create_virtual_world() {
...
void StimulusCylinderGrating_centered_bw::set_grating_info(int i, GratingParams &new_values) {
    GratingType &grating = _cyl.gratings[i];
    {
        OpenThreads::ScopedLock<OpenThreads::Mutex> lock(grating.mutex);
...
POCO_EXPORT_CLASS(StimulusCylinderGrating_centered_bw)
POCO_END_MANIFEST
